...
public:
	...
	bool isDAG();
	bool hasEdge(T &originInfo,T & destInfo) const;
...
template<class T>
bool Graph<T>::hasEdge(T& originInfo, T& destInfo) const {
	Vertex<T> *vertex=getVertex(originInfo);
	if(vertex!=NULL){
		for(int i=0;i<vertex->adj.size();i++){
			if(vertex->adj[i].dest->info==destInfo){
				return true;
			}
		}
		return false;
